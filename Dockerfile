...
# Use x11docker to run GUI apps in docker.
# Get x11docker script from github: https://github.com/mviereck/x11docker
#
# NOTE: Download of Google Earth is disabled in this Dockerfile,
# because it is not allowed by copyright to publish 
# the binaries on Docker Hub. But it is allowed to 
# download them during your build.
#
...
RUN wget --no-check-certificate -P /opt/google/earth/free.newlibs   https://github.com/mviereck/dockerfile-x11docker-google-earth/releases/download/v0.3.0-alpha/ge7.1.1.1580-0.x86_64-new-qt-libs-debian7-ubuntu12.tar.xz
...
RUN mkdir -p /etc/skel/.config/Google
RUN echo '[General]            \n\
enableTips=false               \n\
UsageStats=false               \n\
[TourGuide]                    \n\
...
' > /etc/skel/.config/Google/GoogleEarthPlus.conf
...
case $DISPLAY in                                                              \n\
  "")  echo "Need X server to start Google Earth.                             \n\
